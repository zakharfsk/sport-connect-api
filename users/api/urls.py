...
urlpatterns = [
    ...
    path(
        "auth/token/refresh/",
        extend_schema_view(post=extend_schema(tags=["Authorization"]))(
            TokenRefreshView.as_view()
        ),
        name="token_refresh",
    ),
    path("schools/", views.ListAvailableSchoolsView.as_view(), name="list_schools"),
    path("sports/", views.ListSportView.as_view(), name="list_sports"),
    path(
        "sports/<uuid:sport_id>/schools/",
        views.ListSportSchoolsView.as_view(),
        name="list_sport_schools",
    ),
